...
{
    public static double CalculateConstructionCost(BuildingDefinition buildingDefinition, int currentLevel)
    {
        double baseCost = buildingDefinition.BaseConstructionCost;

        double evolutionMultiplier = BuildingRules.EvolutionCostMultiplier(buildingDefinition.Type);
        double baseConstructionFee = BuildingRules.GetConstructionFee(buildingDefinition.Type);

        double structuralCost = baseCost * Math.Pow(evolutionMultiplier, currentLevel) + baseConstructionFee;

        return structuralCost
               + CalculatePayroll(buildingDefinition)
               + CalculateMaterialCosts(buildingDefinition);
    }
